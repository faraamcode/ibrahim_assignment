...
export const Filter: React.FC<FilterProps> = ({ closeModal }) => {
  ...
  return (
    <FilterWrapper>
      <FilterHeaderStyle>
        <h3>Filter</h3>
        <img onClick={closeModal} src={close} alt='adele' style={{ cursor: 'pointer' }} />
      </FilterHeaderStyle>
      <DateWrapper>
        <p>Today</p>
        <p>Last 7 days</p>
      ...
      <MultiSelectionStyle>
        <p>
          {transactionType.map(item => item).join(', ').length < 45
            ? transactionType.map(item => item).join(', ')
            : `${transactionType
                .map(item => item)
                .join(', ')
                .slice(0, 45)}...`}
      ...
      <CheckboxWrapper>
        ...
        <div>
          ...
          />{' '}
          <label>Store Transaction</label>
        </div>
      </CheckboxWrapper>
      <FilterHeading>Transaction Status</FilterHeading>
      <MultiSelectionStyle>
        <p>
          {transactionStatus.map(item => item).join(', ').length < 45
            ? transactionStatus.map(item => item).join(', ')
            : `${transactionStatus
                .map(item => item)
                .join(', ')
                .slice(0, 45)}...`}
        </p>
        <img src={arrow}  alt='paur'/>
      </MultiSelectionStyle>
      <CheckboxWrapper>
        <div>
          <input
      ...
      <ButtonWrapper>
        ...
        <SmallButton
          text='Apply'
          background={
            transactionStatus.length && transactionType.length
              ? ''
              : 'rgba(0,0,0,0.2)'
          }
        />
